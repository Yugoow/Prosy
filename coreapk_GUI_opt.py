...
class Toolbar(tk.Menu):
    ...
    def donothing(self):
        self.filepath = tkfld.askopenfilename(title="Open a database",
...
class Navbar(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.navTree_yscrb = ttk.Scrollbar(self, orient='vertical',
                                           command=self.navTree.yview)  # yscrb = vertical (y) scrollbar
    ...
    def set_navTree(self):
        ...
        # Firts level (years)
        self.navTree_yearWidg = [self.navTree.insert('', i, self.id_years[i - 1], text='Y' + str(i), tags='A') for i in
                                 range(1, 6)]
        """ Same as :
        i=0
        ...
        for i in range(len(self.navTree_yearWidg)):
            for j in range(1, len(self.id_semesters) + 1):
                ...
                self.navTree.insert(self.navTree_yearWidg[i], 'end', self.id_years[i] + self.id_semesters[j - 1],
                                    text='Semester ' + str(j), tags='S')
    ...
    def set_navCblk(self):
        ...
        # Radiobuttons
        self.navValid_radbS1 = tk.Radiobutton(self.navC_blk, text="Year 1 - Semester 1", variable=self.navCblk_idCylce,
                                              value='_y1_s1', bg=self.bg)
        self.navValid_radbS2 = tk.Radiobutton(self.navC_blk, text="Year 1 - Semester 2", variable=self.navCblk_idCylce,
                                              value='_y1_s2', bg=self.bg)
    ...
    def set_navTreeCh(self):
        ...
        for y_id in self.id_years:
            ch_numberYear = len(self.navTree.get_children(y_id + self.id_semesters[0])) + len(
                self.navTree.get_children(y_id + self.id_semesters[1]))
            # Tree maj
    ...
    def create_navBlk(self):
        ...
        if navCblk_idCylce:
            try:
                ...
                self.navTree.insert(navCblk_idCylce, 'end', self.navSearch_value + navCblk_idCylce,
                                    text=self.navSearch_value)
...
class Outarea(tk.Frame):
    ...
    def start_sim(self):
        global text_sim
        self.get_valAvg(self.get_valNotes())
        coeff_lft= 0
        coeff_uni = []
        ...
        for w in range(len(all_comb)):
            ...
            moy_new = (self.tot_note + tot_inlist) / (self.tot_val + coeff_lft)
    ...
    def get_valAvg(self, rows):
        ...
        self.tot_note: int = 0
        self.tot_val = rows[0][4]

        for row in rows:
            self.tot_note += (row[2] * row[3])
        avg = self.tot_note / self.tot_val
    ...
    def del_block(self):
        ...
        if tkmsg.askyesno("Delete", "Do you really want to delete " + blockIn_name + " ?"):
            if Db_handling.del_dbBlock("", blockIn_name):
                selfNav.delete_navTreeBlk()
                selfNav.set_navTreeCh()
    ...
    def rename_valEty(self, *event):
        ...
        if self.newname and self.newname != '':
            ...
            selfNav.rename_navTreeBlk(self.newname)
            self.error_msg.set(blockIn_name + " is now " + self.newname + " !")
            selfNav.get_navTreeFoc()
...
class Actbar(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        # Button view notes
        self.actBtn_view = tk.Button(self, padx=5, pady=2, text="View",
                                     command=lambda: self.get_action(act_frames[1], 1))
        ...
        self.actBtn_sim = tk.Button(self, padx=5, pady=2, text="Simulation",
                                    command=lambda: self.get_action(act_frames[2], 4))
    ...
    def get_action(self, widget, act, *arg):
        ...
        # if other action
            ...
            # if aciton is view
            if act == 1:
                Outarea.set_view(self.outneighb)

            # if aciton is simulation
            elif act == 4:
                self.outneighb.outSim_spn.focus_set()
...
class MainWindow:
    def __init__(self, master):
        ...
        self.btn_quit = tk.Button(self.master, text="Quit", command=lambda: quit_app(self.master), width=10,
                                  justify="center")
...
class Db_handling:
    ...
    def Db_creation(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'CREATE TABLE blocs(id INTEGER PRIMARY KEY NOT NULL, name VARCHAR(30), semestre VARCHAR(10), year VARCHAR(10));')

            cursor.execute(
                'CREATE TABLE conversion(id INT PRIMARY KEY UNIQUE NOT NULL, denomination CHAR(1), value INT);')

            cursor.execute(
                'CREATE TABLE notes(id INTEGER PRIMARY KEY UNIQUE NOT NULL, id_bloc INT, note CHAR(1), coef INT, FOREIGN KEY (id_bloc) REFERENCES blocs(id));')

            cursor.execute(
                'INSERT INTO conversion(id,denomination,value) VALUES (1,"A",5),(2,"B",4),(3,"C",2),(4,"D",1);')
    ...
    def req_dbNote(self, name):
        id_blk = Db_handling.get_dbIdBlk("", name)
        cursor.execute(
            "SELECT notes.note, notes.id, conversion.value, notes.coef,SUM(notes.coef) OVER() from notes INNER JOIN conversion WHERE notes.note=conversion.denomination and notes.id_bloc=? GROUP BY notes.id",
            (id_blk,))
        rows = cursor.fetchall()
        return rows
